...
public class BeanWrapperFieldSetMapperCustom<T> extends BeanWrapperFieldSetMapper<T> {
    ...
    protected void initBinder(DataBinder binder) {
        ...
        binder.registerCustomEditor(Device.class, new PropertyEditorSupport() {
            ...
            public void setAsText(String text) throws IllegalArgumentException {
                if (StringUtils.isNotEmpty(text)) {
                    Device d = deviceRepository.findById(Long.parseLong(text)).get();
                    setValue(d);
                } else {
                    /*System.out.println("Device by id: " + deviceRepository.findById(1l).get());*/
                    setValue(null);
                }
            }
        ...
        binder.registerCustomEditor(Tester.class, new PropertyEditorSupport() {
            ...
            public void setAsText(String text) throws IllegalArgumentException {
                if (StringUtils.isNotEmpty(text)) {
                    setValue(testerRepository.findById(Long.parseLong(text)).get());
                } else {
                    setValue(1);
                }
            }
